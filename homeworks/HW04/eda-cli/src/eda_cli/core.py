...
def compute_quality_flags(
    ...
    flags["high_duplicate_values_ratio"] = False
    flags["has_many_zero_values"] = False

    if df is not None:
        #Проверка на константные колонки
        for col_summary in summary.columns:
            if col_summary.unique <= 1 and col_summary.non_null > 0:
                flags["has_constant_columns"] = True
                break

        #Проверка на категориальные признаки с высокой кардинальностью
        #Порог: если unique > 0.5 * n_rows, то это высокая кардинальность
        cardinality_threshold = max(50, int(summary.n_rows * 0.5))
        for name in df.columns:
            s = df[name]
            if ptypes.is_object_dtype(s) or isinstance(s.dtype, pd.CategoricalDtype):
        ...
        #Проверка на большое количество дубликатов
        #Допустим, если > 70% значений повторяются, это может быть признаком проблемы
        max_value_duplicates = 0.0
        for col_summary in summary.columns:
            if col_summary.unique > 0 and col_summary.non_null > 0:
                duplicate_ratio = 1.0 - (col_summary.unique / col_summary.non_null)
                max_value_duplicates = max(max_value_duplicates, duplicate_ratio)
        flags["high_duplicate_values_ratio"] = max_value_duplicates > 0.7

        #Проверка на много нулевых значений в числовых колонках
        #Возьмем, что если > 50% значений в числовой колонке равны нулю, это подозрительно
    ...
    #Изменим «скор» качества с учетом новых факторов
    score = 1.0
    ...
    if flags["has_constant_columns"]:
        score -= 0.15  #константные колонки снижают качество
    if flags["has_high_cardinality_categoricals"]:
        score -= 0.1  #высокая кардинальность затрудняет работу
    if flags["high_duplicate_values_ratio"]:
        score -= 0.05  #много дубликатов
    if flags["has_many_zero_values"]:
        score -= 0.05  #много нулей
